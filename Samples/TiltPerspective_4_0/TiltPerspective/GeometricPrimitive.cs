...
namespace TiltPerspectiveSample
{
    ...
    public abstract class GeometricPrimitive : IDisposable
    {
        ...
        public Vector3 LightDirection
        {
            set
            {
                basicEffect.DirectionalLight0.Direction = Vector3.Normalize(value);
            }
        }
        ...
        public void Draw(Matrix world, Matrix view, Matrix projection, Color color, Boolean drawShadow )
        {
            ...
            // Retrieve the current state object details 
            BlendState        tmpBlendState = new BlendState();
            DepthStencilState tmpDepthState = new DepthStencilState();

            tmpDepthState.DepthBufferEnable = true; 
            tmpDepthState.DepthBufferFunction = CompareFunction.LessEqual;
            if (color.A < 255)
            {
                tmpBlendState.AlphaBlendFunction = BlendFunction.Add;
                tmpBlendState.AlphaSourceBlend = Blend.SourceAlpha;
                tmpBlendState.AlphaDestinationBlend = Blend.InverseSourceAlpha;
                tmpDepthState.DepthBufferWriteEnable = false;
            } 
            else 
            {
                tmpDepthState.DepthBufferWriteEnable = true;
            }

            // Apply the updates state objects to the effect
            basicEffect.GraphicsDevice.DepthStencilState = tmpDepthState;
            basicEffect.GraphicsDevice.BlendState = tmpBlendState;
