...
namespace MarbleMazeGame
{
    class Marble : DrawableComponent3D
    {
        ...
        {
            // Calculate roll sound the pitch according to velocity
            float volumeX = MathHelper.Clamp(Math.Abs(Velocity.X) / 400, 0, 1);
            float volumeZ = MathHelper.Clamp(Math.Abs(Velocity.Z) / 400, 0, 1);
            float volume = Math.Max(volumeX, volumeZ);
            float pitch = volume - 1.0f;

            // Play the roll sound only if the marble is actually rolling
            if (intersectDetails.IntersectWithGround && (Velocity.X != 0 || Velocity.Z != 0))
            {
                if (AudioManager.Instance["rolling"].State != SoundState.Playing)
                    AudioManager.PlaySound("rolling", true);

                // Update the volume & pitch by the velocity
                AudioManager.Instance["rolling"].Volume = volume;
            ...
            // Play fall sound when appropriate
            if (Position.Y < -50)
            {
                AudioManager.PlaySound("pit");
            }

            // Play collision sound after a collision
            if (intersectDetails.IntersectWithWalls)
            {
                AudioManager.PlaySound("collision");
                AudioManager.Instance["collision"].Volume = Math.Max(volumeX, volumeZ);
            }
        }
