...
namespace RolePlaying
{
    ...
    class CombatEngine
    {
        ...
        {
            ScaledVector2.GetScaledVector(850f, 345f),
            ScaledVector2.GetScaledVector(980f, 260f),
            ScaledVector2.GetScaledVector(940f, 440f),
            ScaledVector2.GetScaledVector(1100f, 200f),
            ScaledVector2.GetScaledVector(1100f, 490f)
        };
        ...
        {
            ScaledVector2.GetScaledVector(480f, 345f),
            ScaledVector2.GetScaledVector(345f, 260f),
            ScaledVector2.GetScaledVector(370f, 440f),
            ScaledVector2.GetScaledVector(225f, 200f),
            ScaledVector2.GetScaledVector(225f, 490f)
        };
            ...
            public virtual void Draw(SpriteBatch spriteBatch, Texture2D texture)
            {
                ...
                if (texture != null)
                {
                    spriteBatch.Draw(texture, position, null, Color.White, 0f,
                        new Vector2(texture.Width * ScaledVector2.DrawFactor / 2, 
                            texture.Height * ScaledVector2.DrawFactor / 2)
                        , ScaledVector2.DrawFactor,
                        SpriteEffects.None, 0.3f * (float)Rise / 200f * ScaledVector2.ScaleFactor);
                }
                // draw the text
                if (!String.IsNullOrEmpty(Text))
                {
                    spriteBatch.DrawString(Fonts.DamageFont, text, position - new Vector2(5,5),
                        Color.White, 0f, new Vector2(textOrigin.X, textOrigin.Y), 0.85f,
                        SpriteEffects.None, 0.2f * (float)Rise / 200f * ScaledVector2.ScaleFactor);
                }
            }
        ...
        private void CreateSelectionSprites()
        {
            ...
            Point frameDimensions = new Point(36, 27);
            ...
            frameDimensions = new Point(48, 25);
 ...
 primaryTargetedCombatant.Character.Name + "\n" +
                    ...
                    primaryTargetedCombatant.Character.CharacterStatistics.HealthPoints,
                        ...
                        primaryTargetedCombatant.Character.Name,
#endif
 primaryTargetedCombatant.Position + ScaledVector2.GetScaledVector(0f, 42f),
                        Color.White);
                }
            }
 ...
 combatant.Character.Name + "\n" +
                        ...
                        combatant.Character.Name,
#endif
 combatant.Position + ScaledVector2.GetScaledVector(0f, 42f), Color.White);
                }
            }
        }
            ...
            List<CombatantMonster> monsters, int fleeThreshold)
        {
            ...
            AudioManager.PushMusic(TileEngine.Map.CombatMusicCueName,true);
        }
        ...
        private void HandleInput()
        {
            ...
            {
                var touch = InputManager.CurrentTouchState[InputManager.CurrentTouchState.Count - 1];
                foreach (var monster in monsters)
                {
                    var monsterRect =
                    new Rectangle(
                        (int)monster.Position.X - monster.Monster.CombatSprite.FrameDimensions.X / 2,
                        (int)monster.Position.Y - monster.Monster.CombatSprite.FrameDimensions.Y / 2,
                        monster.Monster.CombatSprite.FrameDimensions.X,
                        monster.Monster.CombatSprite.FrameDimensions.Y);
                    if (monsterRect.Contains((int)touch.Position.X, (int)touch.Position.Y))
                    {
                        monsterClicked = true;
                        selectedIndex = index;
                        break;
            ...
            if (InputManager.CurrentTouchState.Count > 0)
            {
                ...
                {
                    Vector2 playerPosition = Hud.PlayerPositionMapping[player];
                    Rectangle playerRect = new Rectangle((int)playerPosition.X, (int)playerPosition.Y,
                        120, 120);
                    if (playerRect.Contains((int)touch.Position.X, (int)touch.Position.Y))
                    {
                        playerClicked = true;
                        selectedIndex = index;
                        break;
            ...
            if (highlightedCombatant.CombatAction != null)
            {
                ...
                if (InputManager.Gestures.Count > 0 &&
                    InputManager.Gestures[InputManager.Gestures.Count -1].GestureType == GestureType.Tap)
                {
                ...
                if (monsterClicked)
                {
                    if (monsterClicked)
                    {
                        if (!monsters[selectedIndex].IsDeadOrDying)
                        {
                            highlightedCombatant.CombatAction.Target = monsters[selectedIndex];
                        }
                        return;
                    }
                ...
                else if (/*InputManager.IsActionTriggered(InputManager.Action.TargetDown) ||*/ monsterClicked)
                {
                    if (monsterClicked)
                    {
                        if (!monsters[selectedIndex].IsDeadOrDying)
            ...
            else // choosing which character will act
            {
                ...
                if (/*InputManager.IsActionTriggered(
                    InputManager.Action.ActiveCharacterLeft) ||*/ playerClicked)
                {
                    if (playerClicked)
                    {
                        BeginPlayerTurn(players[selectedIndex]);
                        return;
                    }
                ...
                // move to the next living character
                    ...
                    InputManager.Action.ActiveCharacterRight) || */playerClicked)
                {
                    if (playerClicked)
                    {
                        BeginPlayerTurn(players[selectedIndex]);
