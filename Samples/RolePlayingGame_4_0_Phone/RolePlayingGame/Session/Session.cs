...
namespace RolePlaying
{
    class Session
    {
        ...
        public static void EncounterFixedCombat(MapEntry<FixedCombat> fixedCombatEntry)
        {
            ...
            foreach (var item in TileEngine.Map.FixedCombatEntries)
            {
                Map.encounterCombat = true;
                Map.LoadCombatIfNecessary(ScreenManager.Game.Content, item);
            }
        ...
        public static void EncounterPortal(MapEntry<Portal> portalEntry)
        {
            ...
            var screen = new SwitchMapScreen(mapContentName, portalEntry.Content.DestinationMapPortalName);
            screen.DoneLoading += new EventHandler(screen_DoneLoading);
            IsChangingMap = true;
            ScreenManager.AddScreen(screen);

        }

        static void screen_DoneLoading(object sender, EventArgs e)
        {
            IsChangingMap = false;
        }
        ...
        private Session(ScreenManager screenManager, GameplayScreen gameplayScreen)
        {
            ...
            hud.StatClicked += new EventHandler(hud_StatClicked);
            this.hud.LoadContent();
        }

        void hud_StatClicked(object sender, EventArgs e)
        {
            TileEngine.StopMovement();
            screenManager.AddScreen(new StatisticsScreen());
        }
        ...
        public static void Draw(GameTime gameTime)
        {
            ...
            if (CombatEngine.IsActive)
            {
                ...
                {
                    spriteBatch.Begin();
                    spriteBatch.Draw(TileEngine.Map.CombatTexture, Vector2.Zero, null, Color.White, 0f,
        ...
        private void DrawShadows(SpriteBatch spriteBatch)
        {
            ...
            if (player.ShadowTexture != null)
            {
                spriteBatch.Draw(player.ShadowTexture,
                    ...
                    new Vector2(
                        ((player.ShadowTexture.Width * ScaledVector2.DrawFactor - TileEngine.Map.TileSize.X * ScaledVector2.DrawFactor)) / 2,
                        ((player.ShadowTexture.Height * ScaledVector2.DrawFactor - TileEngine.Map.TileSize.Y * ScaledVector2.DrawFactor)) / 2 -
                            (player.ShadowTexture.Height * ScaledVector2.DrawFactor) / 6),
                    ScaledVector2.DrawFactor, SpriteEffects.None, 1f);
            }
            ...
            foreach (MapEntry<Player> playerEntry in TileEngine.Map.PlayerNpcEntries)
            {
                ...
                if (playerEntry.Content.ShadowTexture != null)
                {
                    ...
                    spriteBatch.Draw(playerEntry.Content.ShadowTexture, position,
                        null, Color.White, 0f,
                        new Vector2(
                        ((playerEntry.Content.ShadowTexture.Width * ScaledVector2.DrawFactor) -
                            TileEngine.Map.TileSize.X * ScaledVector2.DrawFactor) / 2,
                        ((playerEntry.Content.ShadowTexture.Height * ScaledVector2.DrawFactor) -
                            TileEngine.Map.TileSize.Y * ScaledVector2.DrawFactor) / 2 -
                            (playerEntry.Content.ShadowTexture.Height * ScaledVector2.DrawFactor) / 6),
                        ScaledVector2.DrawFactor, SpriteEffects.None, 1f);
                }
            }
            ...
            foreach (MapEntry<QuestNpc> questNpcEntry in TileEngine.Map.QuestNpcEntries)
            {
                ...
                if (questNpcEntry.Content.ShadowTexture != null)
                {
                    ...
                    spriteBatch.Draw(questNpcEntry.Content.ShadowTexture, position,
                        null, Color.White, 0f,
                        new Vector2(
                            ((questNpcEntry.Content.ShadowTexture.Width * ScaledVector2.DrawFactor) -
                                TileEngine.Map.TileSize.X * ScaledVector2.DrawFactor) / 2,
                            ((questNpcEntry.Content.ShadowTexture.Height * ScaledVector2.DrawFactor) -
                                TileEngine.Map.TileSize.Y * ScaledVector2.DrawFactor) / 2 -
                               (questNpcEntry.Content.ShadowTexture.Height * ScaledVector2.DrawFactor) / 6),
                        ScaledVector2.DrawFactor, SpriteEffects.None, 1f);
                }
            }
                ...
                TileEngine.Map.FixedCombatEntries)
            {
                ...
                if (monster.ShadowTexture != null)
                {
                    ...
                    spriteBatch.Draw(monster.ShadowTexture, position,
                        null, Color.White, 0f,
                        new Vector2(
                        ((monster.ShadowTexture.Width * ScaledVector2.DrawFactor) -
                        TileEngine.Map.TileSize.X * ScaledVector2.DrawFactor) / 2,
                        ((monster.ShadowTexture.Height * ScaledVector2.DrawFactor) -
                        TileEngine.Map.TileSize.Y * ScaledVector2.DrawFactor) / 2 -
                            (monster.ShadowTexture.Height * ScaledVector2.DrawFactor) / 6),
                        ScaledVector2.DrawFactor, SpriteEffects.None, 1f);
                }
            }
        }
            ...
            ScreenManager screenManager, GameplayScreen gameplayScreen)
        {
            ...
            ContentManager content = singleton.screenManager.Game.Content;
            ContentManager staticContent = (singleton.screenManager.Game as RolePlayingGame).StaticContent;
            singleton.party = new Party(gameStartDescription, content, staticContent);

            // load the quest line
            singleton.questLine = staticContent.Load<QuestLine>(
            ...
            ScreenManager screenManager, GameplayScreen gameplayScreen)
        {
            ...
            GetStorageDevice(
             delegate()
             {
                 LoadSessionResult(saveGameDescription);
             });
        ...
        /// <param name="saveGameDescription">The description of the save game.</param>
        public static void LoadSessionResult(SaveGameDescription saveGameDescription)
        {
            // check the parameters
            if (saveGameDescription == null)
            {
            ...
            // open the container
            using (IsolatedStorageFile container = IsolatedStorageFile.GetUserStoreForApplication())
            {
                using (Stream stream =
                    container.OpenFile(saveGameDescription.FileName, FileMode.Open))
                {
                    using (XmlReader xmlReader = XmlReader.Create(stream))
                    {
                        ...
                        singleton.questLine = content.Load<QuestLine>(
                            xmlReader.ReadElementString("questLineContentName")).Clone() as QuestLine;
                        ...
                        singleton.party = new Party(new XmlSerializer(
                            ...
                            (ScreenManager.Game as RolePlayingGame).StaticContent);
        ...
        public static void SaveSession(SaveGameDescription overwriteDescription)
        {
            ...
            GetStorageDevice(delegate()
            {
                SaveSessionResult(overwriteDescription);
            });
        }
        ...
        /// The description of the save game to over-write, if any.
        /// </param>
        private static void SaveSessionResult(SaveGameDescription overwriteDescription)
        {
            using (IsolatedStorageFile container = IsolatedStorageFile.GetUserStoreForApplication())
            {
                string filename;
                ...
                if (overwriteDescription == null)
                {
                    ...
                    while (container.FileExists(testFilename));
                ...
                using (IsolatedStorageFileStream stream = new IsolatedStorageFileStream
                                                                    (filename, FileMode.Create, container))
                {
                ...
                using (Stream stream =
                    container.OpenFile(descriptionFilename, FileMode.Create))
                {
        ...
        public static void DeleteSaveGame(SaveGameDescription saveGameDescription)
        {
            ...
            delegate()
            {
                DeleteSaveGameResult(saveGameDescription);
            });
        }
        ...
        /// <param name="storageDevice">The chosen storage device.</param>
        /// <param name="saveGameDescription">The description of the save game.</param>
        public static void DeleteSaveGameResult(SaveGameDescription saveGameDescription)
        {
            // check the parameters
            if (saveGameDescription == null)
            {
            ...
            using (IsolatedStorageFile container = IsolatedStorageFile.GetUserStoreForApplication())
            {
                container.DeleteFile(saveGameDescription.FileName);
                container.DeleteFile("SaveGameDescription" +
                    Path.GetFileNameWithoutExtension(
        ...
        private static void RefreshSaveGameDescriptionsResult()
        {
            ...
            using (IsolatedStorageFile container = IsolatedStorageFile.GetUserStoreForApplication())
            {
                ...
                string[] filenames =
                    container.GetFileNames();
                // add each entry to the list
                foreach (string filename in filenames)
                {
                    if (!filename.Contains("SaveGameDescription"))
                    {
                        continue;
                    }
        ...
        /// </summary>
        public delegate void StorageDeviceDelegate();
        ...
        public static Dictionary<string, string> MapCache = new Dictionary<string, string>();
        ...
        public static void SaveMapState()
        {
            ...
            if (MapCache.ContainsKey(TileEngine.Map.AssetName))
            {
                MapCache[TileEngine.Map.AssetName] = sb.ToString();
            }
            else
            {
                MapCache.Add(TileEngine.Map.AssetName, sb.ToString());
            }


        }

        public static bool LoadMapState(string mapName)
        {
            bool hasMap = false;
            if (MapCache.ContainsKey(mapName))
            {
                ...
                MemoryStream ms = new MemoryStream(Encoding.Unicode.GetBytes(MapCache[mapName].ToString()));
                using (XmlReader xmlReader = XmlReader.Create(ms))
                {
                    ...
                    {
                        if (item.Content.IconTexture.IsDisposed)
                        {
                            item.Content.IconTexture.Dispose();
                            item.Content.IconTexture = null;
                            item.Content.IconTexture = 
                                (ScreenManager.Game as RolePlayingGame).StaticContent.Load<Texture2D>(
                                @"Textures\Gear\" + item.Content.IconTextureName);
                        }
                    }
                }
            }
