...
namespace RolePlaying
{
    ...
    class StoreBuyScreen : ListScreen<Gear>
    {
        ...
        private static readonly float nameColumnInterval = 80 * ScaledVector2.ScaleFactor;

        private string powerColumnText = "Power (min, max)";
        private static readonly float powerColumnInterval = 270 * ScaledVector2.ScaleFactor;

        private string quantityColumnText = "Qty";
        private static readonly float quantityColumnInterval = 340 * ScaledVector2.ScaleFactor;

        private string priceColumnText = "Price";
        private static readonly float priceColumnInterval = 120 * ScaledVector2.ScaleFactor;
            ...
            bool isSelected)
        {
            ...
            // draw the icon
            spriteBatch.Draw(entry.IconTexture, drawPosition + iconOffset,null, Color.White,0f,
            ...
            if (isSelected)
            {
                if (priceSingle <= Session.Party.PartyGold)
                {
                    ...
                    spriteBatch.Draw(leftQuantityArrow,new Vector2(quantityPosition.X, quantityPosition.Y - 4),null,
                        Color.White,0f,
                        Vector2.Zero, ScaledVector2.DrawFactor, SpriteEffects.None, 0f);
                    quantityPosition.X += leftQuantityArrow.Width * ScaledVector2.DrawFactor;
                    ...
                    spriteBatch.Draw(rightQuantityArrow,
                        new Vector2(quantityPosition.X, quantityPosition.Y - 4),null,
                        Color.White,0f,
                        Vector2.Zero, ScaledVector2.DrawFactor, SpriteEffects.None, 0f);
                    quantityPosition.X += rightQuantityArrow.Width * ScaledVector2.DrawFactor;
