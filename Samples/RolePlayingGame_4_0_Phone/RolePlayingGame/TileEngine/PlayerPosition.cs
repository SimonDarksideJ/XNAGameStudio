...
namespace RolePlaying
{
    ...
    public class PlayerPosition
    {
            ...
            ref Vector2 TileOffset)
        {
            ...
            while (TileOffset.X > TileEngine.Map.TileSize.X * ScaledVector2.DrawFactor / 2f)
            {
                TilePosition.X++;
                TileOffset.X -= TileEngine.Map.TileSize.X * ScaledVector2.DrawFactor;
            }
            while (TileOffset.X < -TileEngine.Map.TileSize.X * ScaledVector2.DrawFactor / 2f)
            {
                TilePosition.X--;
            ...
            while (TileOffset.Y > TileEngine.Map.TileSize.Y * ScaledVector2.DrawFactor / 2f)
            {
                TilePosition.Y++;
                TileOffset.Y -= TileEngine.Map.TileSize.Y * ScaledVector2.DrawFactor;
            }
            while (TileOffset.Y < -TileEngine.Map.TileSize.Y * ScaledVector2.DrawFactor / 2f)
            {
                TilePosition.Y--;
